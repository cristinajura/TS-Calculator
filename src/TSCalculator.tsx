...
const Calculator = () => {
  ...
  const updateInputDisplay = (e: any) => {
    let value = e.target.value;
    const firstParenthesis = outputDisplay
      .split("")
      .filter((x) => x === "(").length;
    const secondParenthesis = outputDisplay
      .split("")
      .filter((x) => x === ")").length;
    ...
    if (inputDisplay.length > 20) {
      ...
      if (outputDisplay === "" && isOperator.test(value)) {
        ...
        (endsWithOperator.test(outputDisplay) && value.match(isOperator)) ||
        (outputDisplay.slice(-1) === "." && value.match(isOperator))
      ) {
        setInputDisplay(value);
        setOutputDisplay(outputDisplay.slice(0, -1) + value);
  ...
  const handleResult = () => {
    ...
    const secondParenthesis: number = outputDisplay
      ...
      .filter((x) => x === ")").length;
    if (
      (inputDisplay.length === 1 && inputDisplay.match(isOperator)) ||
      (inputDisplay === "" && outputDisplay.slice(-1).match(isOperator))
    ) {
      if (outputDisplay.slice(-2) === "(-") {
        let cutLastSign = outputDisplay.slice(0, -3);
        const result: any =
          Math.round(1000000000 * eval(cutLastSign)) / 1000000000;
      ...
      const result: any =
        Math.round(1000000000 * eval(addParenthesis)) / 1000000000;
      setInputDisplay(result);
      ...
      const result: any =
        Math.round(1000000000 * eval(outputDisplay)) / 1000000000;
      setInputDisplay(result);
      setOutputDisplay(outputDisplay + "=");
    }
  };
  ...
  const handlePlusMinus = () => {
    if (
      ...
      if (startWithMinus.test(inputDisplay)) {
        setInputDisplay(inputDisplay.toString().slice(1));
        setOutputDisplay(
          outputDisplay.slice(0, -(inputDisplay.length + 1)) +
            inputDisplay.toString().slice(1)
        );
      } else if (outputDisplay.length !== inputDisplay.length) {
        setInputDisplay("-" + inputDisplay);
        setOutputDisplay(
          outputDisplay.slice(0, -inputDisplay.length) + "(-" + inputDisplay
        );
      } else {
        setInputDisplay("-" + inputDisplay);
        setOutputDisplay("(-" + inputDisplay);
  ...
  const eraseLast = () => {
    const outputDisplayReplace: string[] = outputDisplay
      ...
      .replaceAll("(-", "(0")
      .split("");
    const operators: string[] = outputDisplayReplace.filter((x) =>
      x.match(isOperator)
    );
    const lastOperatorIndex: number = outputDisplayReplace.lastIndexOf(
      operators[operators.length - 1]
    );
    const openParenthesis: number = outputDisplayReplace.filter(
      (x) => x === "("
    ).length;
    const closeParenthesis: number = outputDisplayReplace.filter(
      (x) => x === ")"
    ).length;

    if (!outputDisplay.includes("=")) {
      if (
        (inputDisplay.length === 1 && outputDisplay.length === 1) ||
